...
fmenu1.add_command(label='Standard',command=lambda:standardcolor('on'))
fmenu1.add_command(label='Random',command=lambda:randomcolor_time==1)
...
if randomcolor_time == 1:
    ...
    rck=randomcolor_k()
    randomcolor_time = 0
    print(str(randomcolor_time))
'''
'''
#Standard color
def standardcolor(state):
    global standardcolor_time
    if state == 'on':
        standardcolor_time = 1
        return'#e6e6e6'
        return standardcolor_time
        print(str(standardcolor()))

while standardcolor_time == 1:
    rca=rcb=rcc=rcd=rce=rcf=standardcolor()
    print(str(standardcolor_time))
    standardcolor_time == 0
    break
'''
...
def ButtonClick(btn): #btn stands for button
    ...
    if btn in '123456789':
        if Content.endswith('0'):
            try:
                Content_save = int(eval(str(Content)))
            except:
    ...
    if btn == '0':
        if Content.endswith(Operators):#Check whelther the end of the content is operator or not
            Content+=btn
            Content=Content[:len(Content)-1]#If yes the program will not allow user input 0
        else:
            Content+=btn
